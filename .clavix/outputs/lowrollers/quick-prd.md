...
**Technical Stack:** Angular 21 frontend with signals-based state management, .NET 10.0 / ASP.NET Core backend with lightweight CQRS, SignalR for real-time game actions, self-hosted LiveKit WebRTC SFU for video, **Azure SQL Database + Redis** for persistence, **SQL Server Database Projects** for schema management, hosted on Azure Container Apps with .NET Aspire orchestration. Architecture uses vertical slices with FSM + event sourcing for the game engine. **Data architecture:** GameTables/GameSessions separation, global Players registry, TableTemplates for saved configs. **Hand evaluation via HoldemPoker.Evaluator NuGet package** (hashtable-cached, few-cycle performance). Performance targets: <100ms action latency, <500ms video latency, <50ms sound latency, 60fps gameplay, 500ms animation completion. Budget target <$50/month using scheduled start/stop.
